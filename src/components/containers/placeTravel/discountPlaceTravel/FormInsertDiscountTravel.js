...
let FormInsertDiscountTravel = ({ getIdPlaceTravel, listIdPlaceTravel}) => {
    ...
    let handleClickInsert = async() => {
        let res = await axios.post("http://localhost:8080/api/place-travel/insert-discount-travel", {
        ...
        if(res.data.insert){
            Swal.fire(
            'Thêm Chiết Khấu Du Lịch Thành Công!!!',
            'You clicked the button!',
            'success'
            )

            getIdPlaceTravel(`insert ${discountID}`)
        }
        else{
            Swal.fire({
            icon: 'error',
            title: 'Thêm Chiết Khấu Du Lịch Thất Bại',
            text: 'Chiết Khấu Du Lịch Đã Tồn Tại!!!',
            })
        }
    }
    ...
    return (
        <div className='form-insert-travel'>
            <span className='title-form'>THÊM CHIẾT KHẤU DU LỊCH</span>
            <div className='container-form'>
                <div className='Form'>
                    ...
                    <Row className="mb-3">
                        ...
                        <Form.Control type="text" placeholder="" value={discountID} onChange = {(event) => setDiscountID(event.target.value)}/>
                        </Form.Group>
...
export default FormInsertDiscountTravel